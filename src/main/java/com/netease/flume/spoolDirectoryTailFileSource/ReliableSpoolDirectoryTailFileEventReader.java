package com.netease.flume.spoolDirectoryTailFileSource;

import static com.netease.flume.spoolDirectoryTailFileSource.SpoolDirectoryTailFileSourceConfigurationConstants.*;
...
import com.netease.flume.spoolDirectoryTailFileSource.SpoolDirectoryTailFileSourceConfigurationConstants.*;
...
public class ReliableSpoolDirectoryTailFileEventReader implements ReliableEventReader {
   ...
   */
  private ReliableSpoolDirectoryTailFileEventReader(File spoolDirectory, String completedSuffix, String ignorePattern,
      String targetPattern, String targetFilename, String trackerDirPath, boolean annotateFileName,
      String fileNameHeader, boolean annotateBaseName, String baseNameHeader, String deserializerType,
      Context deserializerContext, String deletePolicy, String inputCharset, DecodeErrorPolicy decodeErrorPolicy,
    ...
    if (logger.isDebugEnabled()) {
      logger.debug("Initializing {} with directory={}, metaDir={}, " + "deserializer={}",
          new Object[] {ReliableSpoolDirectoryTailFileEventReader.class.getSimpleName(), spoolDirectory, trackerDirPath,
              deserializerType});
    }
  ...
  /**
   * Return the filename which generated the data from the last successful {@link #readEvents(int)} call. Returns null
  ...
  public List<Event> readEvents(int numEvents) throws IOException {
    if (!committed) {
      ...
      /**
       * Function: Monitor current file. Author: Ning Guo Time: 2015-02-27
       */
    ...
    /*
     * It's possible that the last read took us just up to a file boundary. If so, try to roll to the next file, if
     * there is one.
     */
    if (events.isEmpty()) {

      /*
       * Function: monitor current file Author: Ning Guo Time: 2015-02-25
       * 
       * deal with two kinds files: 1. old File: delete or rename ; 2. current fresh File: monitor it;
       */
      if (!isTargetFile(currentFile) // Only CurrentFile is no longer the target, at the meanwhile, next file exists.
        ...
        logger.info("File:{} is no longer a TARGET File, which will no longer be monitored.",
            currentFile.get().getFile().getName());
        retireCurrentFile();
        currentFile = getNextFile();
      }
   ...
   *
   * If these operations fail in a way that may cause duplicate log entries, an error is logged but no exceptions are
   * thrown. If these operations fail in a way that indicates potential misuse of the spooling directory, a
  ...
  private void rollCurrentFile(File fileToRoll) throws IOException {
    ...
    if (dest.exists() && PlatformDetect.isWindows()) {
      /*
       * If we are here, it means the completed file already exists. In almost every case this means the user is
       * violating an assumption of Flume (that log files are placed in the spooling directory with unique names).
       * However, there is a corner case on Windows systems where the file was already rolled but the rename was not
      ...
      if (Files.equal(currentFile.get().getFile(), dest)) {
        ...
        if (!deleted) {
          logger.error(
              "Unable to delete file " + fileToRoll.getAbsolutePath() + ". It will likely be ingested another time.");
        }
      } else {
        String message =
            "File name has been re-used with different" + " files. Spooling assumptions violated for " + dest;
      ...
      if (renamed) {
        ...
        /*
         * If we are here then the file cannot be renamed for a reason other than that the destination file exists
         * (actually, that remains possible w/ small probability due to TOC-TOU conditions).
         */
  ...
  /**
   * Returns the next file to be consumed from the chosen directory. If the directory is empty or the chosen file is not
   * readable, this will return an absent option. If the {@link #consumeOrder} variable is {@link ConsumeOrder#OLDEST}
   * then returns the oldest file. If the {@link #consumeOrder} variable is {@link ConsumeOrder#YOUNGEST} then returns
   * the youngest file. If two or more files are equally old/young, then the file name with lower lexicographical value
   * is returned. If THE {@link #consumeOrder} variable is {@link ConsumeOrder#RANDOM} then returns any arbitrary file
   * in the directory.
   */
   ...
   * 
   * @param file
   * @return {@link FileInfo} for the file to consume or absent option if the file does not exists or readable.
   */
  private Optional<FileInfo> openFile(File file) {
    try {
      ...
      EventDeserializer deserializer = EventDeserializerFactory.getInstance(deserializerType, deserializerContext, in);
  ...
  public static class Builder {
    ...
    public ReliableSpoolDirectoryTailFileEventReader build() throws IOException {
      return new ReliableSpoolDirectoryTailFileEventReader(spoolDirectory, completedSuffix, ignorePattern,
          targetPattern, targetFilename, trackerDirPath, annotateFileName, fileNameHeader, annotateBaseName,
          baseNameHeader, deserializerType, deserializerContext, deletePolicy, inputCharset, decodeErrorPolicy,
          consumeOrder);
    }
