...
public class ReliableTaildirEventReader implements ReliableEventReader {
  ...
  private Map<String, String> targetFilenames;
  ...
  private ReliableTaildirEventReader(Map<String, String> filePaths, Map<String, String> targetFilenames,
      Table<String, String, String> headerTable, String positionFilePath, boolean skipToEnd, boolean addByteOffset)
          throws IOException {
    // Sanity checks
      ...
      while (jr.hasNext()) {
        ...
        for (Object v : Arrays.asList(inode, pos, path)) {
          Preconditions.checkNotNull(v,
              "Detected missing value in position file. " + "inode: " + inode + ", pos: " + pos + ", path: " + path);
        }
  ...
  public List<Long> updateTailFiles(boolean skipToEnd) throws IOException {
    ...
    for (Cell<String, File, Pattern> cell : tailFileTable.cellSet()) {
      ...
      String targetFilename = targetFilenames.get(cell.getRowKey());

      for (File f : getMatchFiles(parentDir, fileNamePattern, targetFilename)) {
        ...
        if (tf == null || !tf.getPath().equals(f.getAbsolutePath())) {
          ...
          if (updated) {
            ...
            if (f.length() < tf.getPos()) {
              logger.info("Pos " + tf.getPos() + " is larger than file size! " + "Restarting from pos 0, file: "
  ...
  private List<File> getMatchFiles(File parentDir, final Pattern fileNamePattern, final String targetFilename) {
    FileFilter filter = new FileFilter() {
      public boolean accept(File f) {
        ...
        if (f.isDirectory() || !fileNamePattern.matcher(fileName).matches()
            || !fileName.toLowerCase().contains(substringOfTargetFile.toLowerCase())) {
          return false;
        }
        return true;
      }
  ...
  // */
  //
  // private boolean isTargetFile(String targetFilename) {
  //
  // SimpleDateFormat dateFormat = new SimpleDateFormat(targetFilename);
  // String substringOfTargetFile = dateFormat.format(new Date());
  //
  // if (targetFilename.toLowerCase().contains(substringOfTargetFile.toLowerCase())) {
  // return true;
  // }
  //
  // return false;
  ...
  public static class Builder {
    ...
    public Builder targetFilenames(Map<String, String> targetFilenames) {
      this.targetFilenames = targetFilenames;
      return this;
    }
    ...
    public ReliableTaildirEventReader build() throws IOException {
      return new ReliableTaildirEventReader(filePaths, targetFilenames, headerTable, positionFilePath, skipToEnd,
          addByteOffset);
    }
  }
