...
public class DirectoryTailSource extends AbstractSource implements Configurable, EventDrivenSource {
  ...
  private static final String DEFAULT_PARSER_MODULE_CLASS = "com.netease.flume.directoryTailSource.MultiLineParserModule";
  ...
  private Map<String, Long> fileInitLengthMap;// 文件初始大小记录，用来定位新建fileSet时的游标初始位置
  ...
  private BlockingQueue<DirectoryTailEvent> eventQueue = new LinkedBlockingQueue<DirectoryTailEvent>(maxEventQueueSize);
  ...
  public void configure(Context context) {
    ...
    for (int i = 0; i < confDirArr.length; i++) {
      ...
      String patternString =
          context.getString(CONFIG_DIRS + CONFIG_SEPERATOR + confDirArr[i] + CONFIG_SEPERATOR + CONFIG_FILE_PATTERN);
      ...
      logger.debug("parsed dirs configure dir : " + confDirArr[i] + ", path : " + path);
    }
  ...
  public void start() {
    ...
    for (Entry<String, DirPattern> entry : dirMap.entrySet()) {
      ...
      try {
        ...
        // 新增部分，文件初始化大小保存
        FileObject[] allChiledfile = fileObject.getChildren();
        for (FileObject chiledFileobject : allChiledfile) {
          if (dirPattern.getFilePattern().matcher(chiledFileobject.getName().getBaseName()).find()) {
            String chiledFildPath = chiledFileobject.getName().getPath();
            File chiledfile = new File(chiledFildPath);
            fileInitLengthMap.put(chiledFildPath, chiledfile.length());
            logger.debug(chiledFildPath + " init length is :" + chiledfile.length());
          }
        }
      } catch (FileSystemException e) {
    ...
    private void fileCreated(FileChangeEvent event) throws FileSystemException {
      ...
      if (fileSet == null) {
        try {
          ...
          fileSet = new FileSet(source, event.getFile(), fileInitLengthMap);
